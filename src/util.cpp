...
int nAnonymizeLightningAmount = 100000;
...
#else
    const char* pszModule = "lightning";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Lightning
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Lightning
    // Mac: ~/Library/Application Support/Lightning
    // Unix: ~/.Lightning
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Lightning";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "lightning.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "lightningd.pid"));
