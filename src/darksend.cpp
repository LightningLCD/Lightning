    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Lightning and make it unusable. To
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeLightningAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fLightningSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f LC CENT - sessionTotalValue %d\n", fLightningSubmitted, sessionTotalValue);
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("lightning-darksend");
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeLightningAmount = (rand() % nLeftToAnon)+3;
                } else {
