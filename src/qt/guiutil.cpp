...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("lightning"))
        return false;
...
{
    // Convert lightning:// to lightning:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("lightning://"))
    {
        uri.replace(0, 12, "lightning:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Lightning.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the ILightningLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query ILightningLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "lightning.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Lightning-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  lightning-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Lightning-Qt"));
