...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLightningAmount"))
        settings.setValue("nAnonymizeLightningAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLightningAmount = settings.value("nAnonymizeLightningAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeLightningAmount"))
        SoftSetArg("-anonymizeLightningamount", settings.value("nAnonymizeLightningAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeLightningAmount:
            nAnonymizeLightningAmount = value.toInt();
            settings.setValue("nAnonymizeLightningAmount", nAnonymizeLightningAmount);
            emit anonymizeLightningAmountChanged(nAnonymizeLightningAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
